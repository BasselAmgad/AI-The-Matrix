...
public class bf_analysis {
    public static void main(String[] args) {
        ...
        String[] arr = new String[] {grid10};
        for (String grid : arr){
//        while (true){String grid = Matrix.genGrid();
            System.out.println(grid);
            Matrix matrix = new Matrix(grid);
            String is = ProblemParser.parseProblem(grid);
            System.out.print(matrix.est_MaxDepth_sol(new State(is))+"\t");
            System.out.print(matrix.old_est_MaxDepth_sol(new State(is))+"\t");
            MatrixConfig.visualize = false;
            String output = SearchStrategy.parse("BF").search(matrix);
        }
    }
}
